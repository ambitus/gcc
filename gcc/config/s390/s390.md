  ...
  "reload_completed"
{
  ...
  for (i = 0; i < count; i++)
    XVECEXP (operands[3], 0, i)
      = gen_rtx_SET (gen_rtx_REG (mode, (regno + i) % 16),
  ...
  "reload_completed && TARGET_ZARCH"
{
  ...
  operands[0] = gen_rtx_REG (DImode, (REGNO (operands[1]) + words - 1) % 16);
  return "lmg\t%1,%0,%S2";
}
...
{
  int words = XVECLEN (operands[0], 0);
  operands[0] = gen_rtx_REG (SImode, (REGNO (operands[1]) + words - 1) % 16);
...
{
  int words = XVECLEN (operands[0], 0);
  operands[0] = gen_rtx_REG (DImode, (REGNO (operands[2]) + words - 1) % 16);
  return "stmg\t%2,%0,%S1";
}
   [(set_attr "op_type" "RSY")
    (set_attr "type"    "stm")])
...
{
  int words = XVECLEN (operands[0], 0);
  operands[0] = gen_rtx_REG (SImode, (REGNO (operands[2]) + words - 1) % 16);
...
(define_expand "allocate_stack"
 ...
 "TARGET_ZOS || TARGET_BACKCHAIN"
{
  s390_allocate_stack (operands[0], operands[1]);
  DONE;
})
...
{
  if (TARGET_ZOS)
    s390_emit_f4sa_epilogue (false);
  else
    s390_emit_epilogue (false);
  DONE;
})
...
{
  if (TARGET_ZOS)
    s390_emit_f4sa_epilogue (true);
  else
    s390_emit_epilogue (true);
  DONE;
})
