		...
		rtx retaddr_reg)
{
  ...
  *call = gen_rtx_CALL (VOIDmode, addr_location, const0_rtx);
  ...
  if (retaddr_reg != NULL_RTX)
    {
      *clobber_ret_reg = gen_rtx_CLOBBER (VOIDmode, retaddr_reg);
      ...
      if (TARGET_ZOS)
	/* z/OS TODO: Can we remove this since we now add the use
	   to the insn itself later?  */
  ...
  if (TARGET_ZOS)
    {
      use_reg (&CALL_INSN_FUNCTION_USAGE (insn), arg_pointer_rtx);
      /* z/OS TODO: maybe add clobbers here for both r1 and the
	 argument slots themselves, for better interoperability
	 with code generated by other toolchains.  */
